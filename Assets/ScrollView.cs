...
public class ScrollView : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (runIt)
        {
            AddDelay(distance, pos, takeButton);
            time += Time.deltaTime;
    ...
    private void AddDelay(float distance, float[] pos, Button btn)
    {
        for (int i = 0; i < pos.Length; i++)
        {
            if (scroll_pos < pos[i] + (distance / 2) && scroll_pos > pos[i] - (distance / 2))
            {
                scrollbar.GetComponent<Scrollbar>().value = Mathf.Lerp(scrollbar.GetComponent<Scrollbar>().value, pos[buttonNumber], 1f * Time.deltaTime);
            }
        }
